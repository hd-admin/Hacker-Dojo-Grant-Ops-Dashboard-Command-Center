...
EOF
  ...
  if echo "$PROMPT" | grep -qi "draft\|proposal"; then
    # When called without --format json, return markdown text (draft generation)
    # When called with --format json, return JSON artifact
    case "$all_args" in
      *"--format json"*)
        cat <<EOF
{
  ...
  "version": 1,
  "timestamp": "$(date -u +%Y-%m-%dT%H:%M:%SZ)",
  "content": "This is a comprehensive draft proposal. Hacker Dojo expands access to technology education and community innovation in Silicon Valley. Hacker Dojo has a 17-year track record of serving the Silicon Valley technology community through its makerspace operations, educational programs, and community events. The proposed project will expand access to technology education and community innovation in Silicon Valley.",
  "sections": [
    {
      "sectionTitle": "Project Vision",
...
EOF
    ...
    # Return markdown for direct draft generation (no --format json)
    cat <<'EOF'
