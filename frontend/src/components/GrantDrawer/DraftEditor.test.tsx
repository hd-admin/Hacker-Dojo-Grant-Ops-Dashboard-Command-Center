...
function makeDetail(overrides: Partial<GrantDetailResponse> = {}): GrantDetailResponse {
  return {
    grant: {
      id: "g1", title: "Test Grant", funder: "Test Foundation", funderShort: "TF",
      award: "$100,000", awardSort: 100000, deadline: "2026-06-15", daysOut: 25,
      fit: 88, tags: ["Education"], status: "draft", statusLabel: "Drafting",
      ...
      checklist: [], sourceCount: 3, groundedDocumentCount: 2,
      draftContent: "Hacker Dojo proposes to expand access to technology education.",
      latestDraftVersion: 1,
    },
    latestDraft: { id: "draft-1", grantId: "g1", version: 1, content: "Hacker Dojo proposes to expand access to technology education.", createdAt: "2026-05-23T10:00:00.000Z", createdBy: "agent" },
    latestRevisionRequest: null, approvalRecord: null, submissionRecord: null, followUps: [],
    workflow: { canGenerateDraft: false, canRequestRevision: false, canApprove: false, canSubmit: false, blockingReason: null },
    ...overrides,
  };
}
...
function makeViewModel(overrides: Partial<GrantDrawerViewModel> = {}): GrantDrawerViewModel {
  return {
    grant: null, latestDraftVersionLabel: "Version 1", latestDraftPreview: "Hacker Dojo proposes to expand access to technology education.",
    showGenerateDraft: false, showRequestRevision: false, showApprove: false, showSubmit: false, submitDisabledReason: null,
    ...overrides,
  };
}
...
describe("DraftEditor", () => {
  const defaultProps = () => {
    ...
    return {
      viewModel, detail, draftEditMode: false, draftEditContent: detail.grant.draftContent ?? "",
      setDraftEditContent: vi.fn(), draftIsDirty: false, draftIsSaving: false,
      draftLastSaved: null as string | null, draftSaveNow: vi.fn(async () => {}),
      draftMarkClean: vi.fn(), setDraftEditMode: vi.fn(),
    };
  };
  ...
  it("shows draft preview text", async () => {
    ...
    expect(container.textContent).toContain("Hacker Dojo proposes to expand access to technology education.");
  });

  it("shows AI badge with grounded document and source counts", async () => {
    const detail = makeDetail({ grant: { ...makeDetail().grant, groundedDocumentCount: 2, sourceCount: 2 } });
    const viewModel = makeViewModel();
  ...
  it("shows version label when no draft preview is available", async () => {
    const detail = makeDetail({ latestDraft: null, grant: { ...makeDetail().grant, draftContent: "" } });
    const viewModel = makeViewModel({ latestDraftPreview: "", latestDraftVersionLabel: "No draft yet" });
    root.render(React.createElement(DraftEditor, { ...defaultProps(), detail, viewModel }));
  ...
  it("shows dirty indicator when draft has unsaved changes", async () => {
    ...
    expect(container.querySelector("[data-testid='draft-dirty-indicator']")?.textContent).toBe("Unsaved");
  });
  ...
  it("shows saving indicator when draft is being saved", async () => {
    ...
    expect(container.querySelector("[data-testid='draft-saving-indicator']")?.textContent).toBe("Saving...");
  });

  it("shows saved timestamp when last saved is set and not dirty", async () => {
    root.render(React.createElement(DraftEditor, { ...defaultProps(), draftIsDirty: false, draftLastSaved: "2026-05-23T10:30:00.000Z" }));
    await waitFor(() => container.querySelector("[data-testid='draft-saved-timestamp']") !== null);
    expect(container.querySelector("[data-testid='draft-saved-timestamp']")?.textContent).toContain("Saved at");
  });
  ...
  it("shows textarea with accessible label in edit mode", async () => {
    ...
    await waitFor(() => container.querySelector("textarea[aria-label='Edit draft content']") !== null);
  });

  it("calls saveNow and disables button while saving", async () => {
    root.render(React.createElement(DraftEditor, { ...defaultProps(), draftEditMode: true, draftIsSaving: true }));
    await waitFor(() => container.querySelector("[data-testid='draft-save-now-btn']") !== null);
    const saveButton = container.querySelector("[data-testid='draft-save-now-btn']") as HTMLButtonElement;
  ...
  it("Done Editing resets to original content and marks clean", async () => {
    ...
    const detail = makeDetail({ grant: { ...makeDetail().grant, draftContent: "Original content" } });
    root.render(React.createElement(DraftEditor, { ...defaultProps(), detail, draftEditMode: true, setDraftEditMode, setDraftEditContent, draftMarkClean }));
    await waitFor(() => container.textContent?.includes("Done Editing") === true);
    const doneButton = Array.from(container.querySelectorAll("button")).find((btn) => btn.textContent === "Done Editing") as HTMLButtonElement;
    doneButton?.click();
  ...
  it("shows revision request status", async () => {
    const detail = makeDetail({ latestRevisionRequest: { id: "rev-1", grantId: "g1", draftVersion: 1, notes: "Please tighten the budget.", requestedAt: "2026-05-23T11:00:00Z", requestedBy: "human", status: "pending" } });
    root.render(React.createElement(DraftEditor, { ...defaultProps(), detail }));
    await waitFor(() => container.textContent?.includes("Last revision note: Please tighten the budget.") === true);
