    ...
    <>
      <div className="drawer-section">
        <h3 className={styles.sectionHeaderFlex}>
          ...
          {draftIsDirty && (
            <span data-testid="draft-dirty-indicator" className={styles.draftDirtyIndicator}>
              Unsaved
            </span>
          )}
          {draftIsSaving && (
            <span data-testid="draft-saving-indicator" className={styles.draftSavingIndicator}>
              Saving...
            </span>
          )}
          {draftLastSaved && !draftIsDirty && (
            <span data-testid="draft-saved-timestamp" className={styles.draftSavedTimestamp}>
              Saved at{' '}
              {new Date(draftLastSaved).toLocaleTimeString([], {
                hour: '2-digit',
                minute: '2-digit',
              })}
            </span>
        ...
        {viewModel.latestDraftPreview ? (
          <div className="draft-meta">
            ...
            <span>
              {(() => {
                const words = viewModel.latestDraftPreview
                  .split(/\s+/)
        ...
        <div className="drawer-note">
          Revision requests:{' '}
          {detail.latestRevisionRequest ? detail.latestRevisionRequest.status : 'none'}
        </div>
      ...
      {detail.latestDraft && <GroundingReview draftArtifact={detail.latestDraft} />}
    </>
  );
}
