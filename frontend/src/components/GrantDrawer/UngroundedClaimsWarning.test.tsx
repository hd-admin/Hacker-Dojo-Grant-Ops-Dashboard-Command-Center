...
describe('UngroundedClaimsWarning', () => {
  const defaultProps = () => ({
    showGroundingWarning: false,
    groundingOverrideConfirmed: false,
    setGroundingOverrideConfirmed: vi.fn(),
    doApproveAndLock: vi.fn(async () => {}),
    setShowGroundingWarning: vi.fn(),
    latestDraft: null as DraftArtifact | null,
  });
  ...
  it('renders nothing when there is no latest draft', () => {
    root.render(
      React.createElement(UngroundedClaimsWarning, {
        ...defaultProps(),
        showGroundingWarning: true,
      }),
    );
    expect(container.querySelector("[data-testid='grounding-warning-dialog']")).toBeNull();
  });
  ...
  it('renders warning dialog when showGroundingWarning is true and draft exists', async () => {
    ...
    await waitFor(
      () => container.querySelector("[data-testid='grounding-warning-dialog']") !== null,
    );
  ...
  it('Approve Anyway button is disabled until checkbox is checked', async () => {
    ...
    await waitFor(
      () => container.querySelector("[data-testid='grounding-approve-anyway-btn']") !== null,
    );
    expect(
      (container.querySelector("[data-testid='grounding-approve-anyway-btn']") as HTMLButtonElement)
        ?.disabled,
    ).toBe(true);
  });
  ...
  it('Approve Anyway button is enabled when checkbox is checked', async () => {
    ...
    root.render(
      React.createElement(UngroundedClaimsWarning, {
        ...
        showGroundingWarning: true,
        latestDraft,
        groundingOverrideConfirmed: true,
      }),
    );
    await waitFor(
      () => container.querySelector("[data-testid='grounding-approve-anyway-btn']") !== null,
  ...
  it('checkbox toggle calls setGroundingOverrideConfirmed', async () => {
    ...
    await waitFor(
      () => container.querySelector("[data-testid='grounding-override-checkbox']") !== null,
    );
    (
      container.querySelector("[data-testid='grounding-override-checkbox']") as HTMLInputElement
    )?.click();
