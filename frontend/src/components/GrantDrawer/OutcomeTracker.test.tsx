...
function makeDetail(overrides: Partial<GrantDetailResponse> = {}): GrantDetailResponse {
  return {
    grant: {
      ...
      fit: 88, tags: ["Education"], status: "review", statusLabel: "Review",
...
function makeAuditEvents(): AuditEvent[] {
  return [
    { id: "evt-1", entityId: "g1", entityType: "grant", eventType: "fit-override", actorLabel: "Operator", timestamp: "2026-05-23T12:00:00.000Z" },
    { id: "evt-2", entityId: "g1", entityType: "grant", eventType: "approval", actorLabel: "Operator", timestamp: "2026-05-23T13:00:00.000Z" },
  ];
}
...
describe("OutcomeTracker", () => {
  const defaultProps = () => {
    ...
    return {
      detail, overrideField: null as "fit" | "category" | "status" | null,
      setOverrideField: vi.fn(), overrideValue: "", setOverrideValue: vi.fn(),
      overrideRationale: "", setOverrideRationale: vi.fn(),
      handleSubmitOverride: vi.fn(async () => {}), auditEvents: makeAuditEvents(),
    };
  };
  ...
  it("shows override buttons", async () => {
    ...
    const buttonTexts = Array.from(container.querySelectorAll(".drawer-actions button")).map((b) => b.textContent);
  ...
  it("opens override panel when override-fit-score-btn is clicked", async () => {
    ...
    root.render(React.createElement(OutcomeTracker, { ...defaultProps(), setOverrideField, setOverrideValue, setOverrideRationale }));
    await waitFor(() => container.querySelector("[data-testid='override-fit-score-btn']") !== null);
  ...
  it("opens category override panel", async () => {
    ...
    root.render(React.createElement(OutcomeTracker, { ...defaultProps(), detail, setOverrideField, setOverrideValue: vi.fn(), setOverrideRationale: vi.fn() }));
    await waitFor(() => container.textContent?.includes("Override category") === true);
    (Array.from(container.querySelectorAll(".drawer-actions button")).find((btn) => btn.textContent === "Override category") as HTMLButtonElement)?.click();
  ...
  it("opens status override panel with select dropdown", async () => {
    ...
    (Array.from(container.querySelectorAll(".drawer-actions button")).find((btn) => btn.textContent === "Override status") as HTMLButtonElement)?.click();
    expect(setOverrideField).toHaveBeenCalledWith("status");
  });

  it("renders override panel with inputs when overrideField is set", async () => {
    root.render(React.createElement(OutcomeTracker, { ...defaultProps(), overrideField: "fit", overrideValue: "75", overrideRationale: "Adjusted" }));
    await waitFor(() => container.querySelector(".override-panel") !== null);
    expect(container.querySelector(".override-panel")?.textContent).toContain("Provide a rationale before saving.");
  });

  it("shows Cancel and Save override buttons in the panel", async () => {
    root.render(React.createElement(OutcomeTracker, { ...defaultProps(), overrideField: "fit", overrideValue: "75", overrideRationale: "Test" }));
    await waitFor(() => container.querySelector(".override-panel") !== null);
    const buttonTexts = Array.from(container.querySelectorAll(".override-panel button")).map((b) => b.textContent);
  ...
  it("calls handleSubmitOverride when Save override is clicked", async () => {
    ...
    root.render(React.createElement(OutcomeTracker, { ...defaultProps(), overrideField: "fit", overrideValue: "91", overrideRationale: "Test rationale", handleSubmitOverride }));
    await waitFor(() => container.textContent?.includes("Save override") === true);
    (Array.from(container.querySelectorAll(".override-panel button")).find((btn) => btn.textContent === "Save override") as HTMLButtonElement)?.click();
  ...
  it("clears override state on Cancel", async () => {
    ...
    const setOverrideValue = vi.fn();
    const setOverrideRationale = vi.fn();
    root.render(React.createElement(OutcomeTracker, { ...defaultProps(), overrideField: "fit", overrideValue: "91", overrideRationale: "Test", setOverrideField, setOverrideValue, setOverrideRationale }));
    await waitFor(() => container.textContent?.includes("Cancel") === true);
    (Array.from(container.querySelectorAll(".override-panel button")).find((btn) => btn.textContent === "Cancel") as HTMLButtonElement)?.click();
  ...
  it("limits audit events to 10", async () => {
    const manyEvents: AuditEvent[] = Array.from({ length: 15 }, (_, i) => ({
      id: `evt-${i}`, entityId: "g1", entityType: "grant", eventType: `event-${i}`,
      actorLabel: `Actor ${i}`, timestamp: `2026-05-${23 + i}T12:00:00.000Z`,
    }));
    root.render(React.createElement(OutcomeTracker, { ...defaultProps(), auditEvents: manyEvents }));
