...
function makeFollowUp(overrides: Partial<FollowUp> = {}): FollowUp {
  return {
    ...
    description: 'Follow up with NSF on status.',
...
describe('FollowUpManager', () => {
  const defaultProps = () => ({
    ...
    setShowFollowUpForm: vi.fn(),
    newFollowUpType: 'other' as FollowUp['type'],
    setNewFollowUpType: vi.fn(),
    newFollowUpTitle: '',
    setNewFollowUpTitle: vi.fn(),
    newFollowUpDescription: '',
    setNewFollowUpDescription: vi.fn(),
    newFollowUpDueDate: '',
    setNewFollowUpDueDate: vi.fn(),
    ...
    outcomeNotes: '',
    setOutcomeNotes: vi.fn(),
    handleSaveOutcome: vi.fn(async () => {}),
    setShowOutcomeForm: vi.fn(),
  ...
  it('shows follow-up form when showFollowUpForm is true', async () => {
    root.render(
      React.createElement(FollowUpManager, { ...defaultProps(), showFollowUpForm: true }),
    );
  ...
  it('disables save button when title is empty', async () => {
    root.render(
      React.createElement(FollowUpManager, {
        ...defaultProps(),
    ...
    expect(
      (container.querySelector("[data-testid='save-follow-up-btn']") as HTMLButtonElement)
        ?.disabled,
    ).toBe(true);
  });
  ...
  it('shows loading state for follow-ups', async () => {
    root.render(
      React.createElement(FollowUpManager, { ...defaultProps(), followUpsLoading: true }),
    );
  ...
  it('renders follow-up items with status badges', async () => {
    ...
    expect(container.querySelector("[data-testid='follow-up-item-fu-1']")?.textContent).toContain(
    ...
    await waitFor(
      () =>
        container.querySelector(
          '[aria-label=\'Mark "Check status of application" as complete\']',
        ) !== null,
    );
  });
  ...
  it('calls handleMarkComplete when mark-complete button is clicked', async () => {
    ...
    root.render(
      React.createElement(FollowUpManager, { ...defaultProps(), followUps, handleMarkComplete }),
    );
    await waitFor(() => container.querySelector("[data-testid='mark-complete-btn-fu-1']") !== null);
    (
      container.querySelector("[data-testid='mark-complete-btn-fu-1']") as HTMLButtonElement
    )?.click();
  ...
  it('shows delete button with accessible label', async () => {
    ...
    await waitFor(
      () =>
        container.querySelector('[aria-label=\'Delete "Check status of application"\']') !== null,
    );
  });
  ...
  it('calls handleDeleteFollowUp when delete button is clicked', async () => {
    ...
    root.render(
      React.createElement(FollowUpManager, { ...defaultProps(), followUps, handleDeleteFollowUp }),
    );
    await waitFor(
      () => container.querySelector("[data-testid='delete-follow-up-btn-fu-1']") !== null,
    );
    (
      container.querySelector("[data-testid='delete-follow-up-btn-fu-1']") as HTMLButtonElement
    )?.click();
  ...
  it('shows outcome form when showOutcomeForm is true and detail is provided', async () => {
    root.render(
      React.createElement(FollowUpManager, {
        ...defaultProps(),
        showOutcomeForm: true,
        detail: { grant: { statusLabel: 'Awarded', id: 'g1' } },
      }),
    );
  ...
  it('disables save outcome button when notes are empty', async () => {
    ...
    await waitFor(() => container.querySelector("[data-testid='save-outcome-btn']") !== null);
    expect(
      (container.querySelector("[data-testid='save-outcome-btn']") as HTMLButtonElement)?.disabled,
    ).toBe(true);
  });
