...
function makeGrant(details: Partial<GrantDetailResponse["grant"]> = {}): GrantDetailResponse["grant"] {
  return {
    id: "g1",
...
describe("GrantDrawerHeader", () => {
  ...
  it("calls onClose when close button is clicked", async () => {
    let closed = false;
    const grant = makeGrant();
    root.render(React.createElement(GrantDrawerHeader, { grant, onClose: () => { closed = true; } }));
    await waitFor(() => container.querySelector("[aria-label='Close']") !== null);
  ...
  it("shows (estimated) badge when deadlineConfidence is estimated", async () => {
    ...
    expect(container.querySelector("[data-testid='deadline-confidence-badge']")?.textContent).toBe("(estimated)");
  });
  ...
  it("shows (date uncertain) badge when deadlineConfidence is unknown", async () => {
    ...
    root.render(React.createElement(GrantDrawerHeader, { grant, onClose: () => {} }));
    await waitFor(() => container.querySelector("[data-testid='deadline-confidence-badge']") !== null);
    expect(container.querySelector("[data-testid='deadline-confidence-badge']")?.textContent).toBe("(date uncertain)");
  });
  ...
  it("shows human-confirmed badge when fit has human overrides", async () => {
    const grant = makeGrant({
      humanOverrides: [{ field: "fit", previousValue: 88, newValue: 91, rationale: "Reviewed", overriddenAt: "2026-01-01T00:00:00Z", overriddenBy: "operator", overrideType: "score" }],
    });
    root.render(React.createElement(GrantDrawerHeader, { grant, onClose: () => {} }));
    await waitFor(() => container.querySelector("[data-testid='fit-human-confirmed-badge']") !== null);
    expect(container.querySelector("[data-testid='fit-human-confirmed-badge']")?.textContent).toContain("Human-confirmed");
  });
