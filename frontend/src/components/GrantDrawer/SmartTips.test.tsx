...
describe('SmartTips', () => {
  const defaultProps = () => {
    ...
    return {
      detail,
      viewModel,
      manifest: null as SubmissionManifest | null,
      manifestLoading: false,
      handleGenerateDraft: vi.fn(async () => {}),
      handleApproveAndLock: vi.fn(async () => {}),
      handleRequestRevision: vi.fn(),
      setShowSubmitForm: vi.fn(),
      handleOpenInEditor: vi.fn(),
      handleViewOnGrantsGov: vi.fn(),
      handleCreateManifest: vi.fn(async () => {}),
      submissionDocuments: [] as DocumentMetadata[],
      handleSubmitComplete: vi.fn(async (_data: unknown) => {}),
      runbookExpanded: false,
      setRunbookExpanded: vi.fn(),
      runbookConfirmationNumber: '',
      setRunbookConfirmationNumber: vi.fn(),
      runbookCompleted: false,
      setRunbookCompleted: vi.fn(),
      handleSaveRunbook: vi.fn(async () => {}),
      runbookSaving: false,
    };
  };
  ...
  it('renders funder summary section', async () => {
    ...
    await waitFor(
      () => container.textContent?.includes('Funder summary (agent-generated)') === true,
    );
  ...
  it('shows manifest details when manifest exists', async () => {
    const manifest = makeManifest({
      ...
      fileConstraints: 'PDF only',
      dueDate: '2026-06-14',
      notes: 'Check budget.',
      materialRefs: [
        { documentId: 'd1', documentName: 'Narrative.pdf', role: 'narrative' },
        { documentId: 'd2', documentName: 'Budget.xlsx', version: 'v2', role: 'budget' },
      ],
    });
  ...
  it('shows Create manifest button when no manifest exists', async () => {
    ...
    const createButton = Array.from(container.querySelectorAll('button')).find(
      (btn) => btn.textContent === 'Create manifest',
    );
  ...
  it('calls handleGenerateDraft when button is clicked', async () => {
    ...
    (
      Array.from(container.querySelectorAll('button')).find(
        (btn) => btn.textContent === 'Generate draft',
      ) as HTMLButtonElement
    )?.click();
  ...
  it('exposes runbook toggle button with proper aria attributes', async () => {
    ...
    await waitFor(() => container.querySelector("[data-testid='runbook-toggle-btn']") !== null);
    expect(
      container.querySelector("[data-testid='runbook-toggle-btn']")?.getAttribute('aria-expanded'),
    ).toBe('false');
    expect(
      container.querySelector("[data-testid='runbook-toggle-btn']")?.getAttribute('aria-controls'),
    ).toBe('submission-runbook-content');
  });
