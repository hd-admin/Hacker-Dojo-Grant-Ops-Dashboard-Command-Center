...
function determineConfidence(section: {
  evidence: string[];
  isGrounded: boolean;
}): ConfidenceLevel {
...
export function GroundingReview({ draftArtifact, onReviewComplete }: GroundingReviewProps) {
  ...
  const [sectionStates, setSectionStates] = useState<GroundingSectionState[]>(sections);
    ...
    (sectionTitle: string) => {
      setSectionStates((prev) => {
        const next = prev.map((s) =>
          s.sectionTitle === sectionTitle ? { ...s, reviewed: !s.reviewed } : s,
        );
    ...
    return (
      <div className="drawer-section">
        ...
        <p className="grounding-empty">
          No grounding sections available for this draft. Grounding data will appear when the draft
          generator provides section-level source metadata.
        </p>
      </div>
    );
  }
  ...
  const weakUnreviewed = sectionStates.filter((s) => s.confidence === 'weak' && !s.reviewed).length;
  ...
  return (
    <div className="drawer-section">
      ...
      <div className="grounding-overall" role="status" aria-live="polite">
        ...
        {allReviewed && (
          <span className="grounding-overall-badge grounding-complete">✓ Review complete</span>
        )}
            ...
            >
              ...
              <div className="grounding-section-header">
                <label className="grounding-review-check">
                  ...
                  />
                  <span className="grounding-check-label">{section.sectionTitle}</span>
                </label>

                <div className="grounding-quality-indicator">
                  <span className={`grounding-quality-dot ${quality.className}`} aria-hidden="true">
                    {quality.indicator}
                  </span>
                  <span className="grounding-quality-label">{quality.label}</span>
                </div>
              ...
              {evidenceItems.length > 0 && (
                <div className="grounding-evidence-summary">
                  ...
                  >
                    {isExpanded ? '▼' : '▶'} Evidence sources ({evidenceItems.length})
                  </button>
                    ...
                    >
                      {evidenceItems.map((item, idx) => (
                        ...
                        >
                          ...
                          <span className="grounding-evidence-text">{item.sourceName}</span>
                        </li>
                      ))}
                    </ul>
                  )}
              ...
              {/* No evidence warning */}
              {evidenceItems.length === 0 && (
                <div className="grounding-no-evidence" role="alert">
                  No linked evidence sources available for this section.
                </div>
              )}
            </div>
          );
      ...
      {unsupportedCount > 0 && (
        <div className="grounding-stale-warning grounding-blocking" role="alert">
          ...
          {unsupportedCount !== 1 ? 's contain' : ' contains'} unsupported claims. All unsupported
          sections must be reviewed or removed before approval.
        </div>
      )}
      {weakCount > 0 && unsupportedCount === 0 && (
        <div className="grounding-stale-warning grounding-acknowledge" role="alert">
          ...
          {weakCount !== 1 ? 's have' : ' has'} weak grounding. Review and acknowledge these gaps
          before approval.
        </div>
      )}
    </div>
