...
import { buildGrantDrawerViewModel, saveWorkingContextField, waitForJobCompletion } from "./GrantDrawer/utilities";
...
export function GrantDrawer({
	...
	const saveDraftContent = useCallback(async (content: string) => {
		...
		setDetail((prev) =>
			prev
				? { ...prev, grant: { ...prev.grant, draftContent: content } }
				: prev,
		);
	}, [grantId]);
	...
	const loadDetail = useCallback(async () => {
		if (!grantId) { setDetail(null); return; }
		setLoading(true);
		try {
	...
	useEffect(() => {
		if (!grantId) { setSubmissionDocuments([]); return; }
	...
	const loadManifest = useCallback(async () => {
		if (!grantId) { setManifest(null); return; }
	...
	useEffect(() => {
		async function loadAuditTrail() {
			if (!grantId) { setAuditEvents([]); return; }
			try {
	...
	useEffect(() => {
		async function loadFollowUps() {
			if (!grantId) { setFollowUps([]); return; }
	...
	const handleGenerateDraft = async () => {
		if (!detail) return;
		if (detail.approvalRecord) { setShowLockedDraftConfirm(true); return; }
	...
	const handleApproveAndLock = async () => {
		...
		if (hasUngroundedSections) { setShowGroundingWarning(true); return; }
	...
	const handleSubmit = async () => {
		...
		try {
			...
			if (submitMethod === "portal" && portalUrl) { method.portalUrl = portalUrl; }
			if (confirmationId) { method.confirmationId = confirmationId; }
	...
	const handleRequestRevision = () => { setShowRevision(true); };

	const handleRequestClose = () => {
		if (hasDirtyNotes || draftIsDirty) { setCloseWarningOpen(true); return; }
		onClose();
	};
	...
	const handleCancelRevision = () => { setShowRevision(false); setRevisionNote(""); };
	...
	const handleOpenInEditor = () => {
		if (detail?.grant.externalUrl) { window.open(detail.grant.externalUrl); }
	};
					...
					<div className="drawer-body">
						...
						{showRevision && (
							<div className="drawer-section">
								...
								<textarea className="form-input" rows={4} value={revisionNote} onChange={(e) => setRevisionNote(e.target.value)} />
								<div>
									<button type="button" className="btn btn-primary" onClick={handleConfirmRevision}>Save revision</button>
									<button type="button" className="btn" onClick={handleCancelRevision}>Cancel</button>
								</div>
							</div>
						)}
						...
						{showSubmitForm && (
							<div className="drawer-section">
								...
								{submitMethod === "portal" && (
									<input type="url" className="form-input" placeholder="Portal URL" value={portalUrl} onChange={(e) => setPortalUrl(e.target.value)} />
								)}
								<input type="text" className="form-input" placeholder="Confirmation ID" value={confirmationId} onChange={(e) => setConfirmationId(e.target.value)} />
								<textarea className="form-input" rows={3} placeholder="Submission notes" value={submitNotes} onChange={(e) => setSubmitNotes(e.target.value)} />
								<div>
									<button type="button" className="btn btn-primary" onClick={handleSubmit}>Submit</button>
									<button type="button" className="btn" onClick={() => setShowSubmitForm(false)}>Cancel</button>
								</div>
							</div>
						)}
						...
						<FollowUpManager
							...
							detail={detail ? { grant: { statusLabel: detail.grant.statusLabel, id: detail.grant.id } } : null}
						/>
