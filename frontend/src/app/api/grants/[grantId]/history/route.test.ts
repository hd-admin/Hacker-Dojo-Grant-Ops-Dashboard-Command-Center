...
describe('/api/grants/[grantId]/history route', () => {
  ...
  it('returns grant transition history', async () => {
    const mockHistory = [
      { id: 't1', grantId: 'g1', fromState: 'matched', toState: 'draft', actor: 'operator', reason: '', createdAt: '2026-01-01' },
    ];
    ...
    const response = await GET(req as unknown as NextRequest, { params: Promise.resolve({ grantId: 'g1' }) });
  ...
  it('returns empty history for grant with no transitions', async () => {
    ...
    const response = await GET(req as unknown as NextRequest, { params: Promise.resolve({ grantId: 'g2' }) });
    const data = await (response as NextResponse).json();
