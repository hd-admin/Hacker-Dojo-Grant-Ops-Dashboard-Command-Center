...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (search) {
      ...
      const { getSqliteState, openDatabase } =
    ...
    if (funderType) {
      filtered = filtered.filter(
        (g) =>
          g.category === funderType || g.funder?.toLowerCase().includes(funderType.toLowerCase()),
      );
    }
    if (minFit) {
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json(createErrorResponse('AGENT_INVALID_JSON', 'Invalid grant payload'), {
        status: 400,
      });
    }
    ...
    return NextResponse.json(createErrorResponse('STORAGE_UNAVAILABLE', 'Failed to create grant'), {
      status: 500,
    });
  }
}
