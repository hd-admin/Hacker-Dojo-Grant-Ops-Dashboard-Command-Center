...
export async function PUT(
  ...
  try {
    ...
    if (idx === -1) {
      return NextResponse.json(createErrorResponse('FILE_NOT_FOUND', 'Saved search not found'), {
        status: 404,
      });
    }
    ...
    return NextResponse.json(
      createErrorResponse('STORAGE_UNAVAILABLE', 'Failed to update saved search'),
      { status: 500 },
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      createErrorResponse('STORAGE_UNAVAILABLE', 'Failed to delete saved search'),
      { status: 500 },
    );
  }
}
