...
describe('/api/awards/[awardId]/compliance route', () => {
  ...
  it('returns compliance items for an award', async () => {
    const mockItems = [{ id: 'c1', awardId: 'a1', requirement: 'Annual report', status: 'pending' }];
  ...
  it('creates a compliance item', async () => {
    ...
    const response = await PUT(req as unknown as NextRequest, { params: Promise.resolve({ awardId: 'a1' }) });
  ...
  it('returns 400 for invalid compliance data', async () => {
    ...
    const req = new Request('http://localhost/api/awards/a1/compliance', {
      method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ status: 'invalid-status' }),
    });
    const response = await PUT(req as unknown as NextRequest, { params: Promise.resolve({ awardId: 'a1' }) });
