...
describe('/api/awards/[awardId]/expenses route', () => {
  ...
  it('creates an expense', async () => {
    ...
    const req = new Request('http://localhost/api/awards/a1/expenses', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ description: 'Item', amount: 500 }) });
    const response = await POST(req as unknown as NextRequest, { params: Promise.resolve({ awardId: 'a1' }) });
    expect(response.status).toBe(201);
  });

  it('returns 400 for invalid expense data', async () => {
    (getDependencies as ReturnType<typeof vi.fn>).mockReturnValue({
      repository: {}, idGenerator: { generateId: (p: string) => `${p}-test` },
    });
    const req = new Request('http://localhost/api/awards/a1/expenses', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ amount: 'not-a-number' }) });
    const response = await POST(req as unknown as NextRequest, { params: Promise.resolve({ awardId: 'a1' }) });
