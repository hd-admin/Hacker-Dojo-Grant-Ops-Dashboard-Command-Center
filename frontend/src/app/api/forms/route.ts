...
export async function GET(_req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    return NextResponse.json(createErrorResponse('DB_INTEGRITY_ERROR', 'Failed to get forms'), {
      status: 500,
    });
  }
}
...
export async function PUT(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const parsed = formSchema.parse(body);
    if (!parsed.id) {
      return NextResponse.json(createErrorResponse('VALIDATION_ERROR', 'Form ID is required'), {
        status: 400,
      });
    }
    const deps = getDependencies();
...
export async function DELETE(req: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const { deleteFormTemplate, getSqliteState } =
    ...
    return NextResponse.json(createErrorResponse('DB_INTEGRITY_ERROR', 'Failed to delete form'), {
      status: 500,
    });
  }
}
