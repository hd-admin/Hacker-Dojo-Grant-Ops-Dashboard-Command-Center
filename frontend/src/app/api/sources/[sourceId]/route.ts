  ...
  try {
    ...
    if (!parsed.success) {
      return NextResponse.json(
        { error: 'Invalid source update payload', issues: parsed.error.flatten() },
        { status: 400 },
      );
    }
    ...
    if (parsed.data.categoryRationale !== undefined)
      updates.categoryRationale = parsed.data.categoryRationale;
    if (parsed.data.crawlAccessCategory !== undefined)
      updates.crawlAccessCategory = parsed.data.crawlAccessCategory;
    if (parsed.data.authMethodDescription !== undefined)
      updates.authMethodDescription = parsed.data.authMethodDescription;
    if (parsed.data.crawlFrequencyRecommendation !== undefined)
      updates.crawlFrequencyRecommendation = parsed.data.crawlFrequencyRecommendation;
    if (parsed.data.operatorNotes !== undefined) updates.operatorNotes = parsed.data.operatorNotes;
    if (parsed.data.lastManualReviewDate !== undefined)
      updates.lastManualReviewDate = parsed.data.lastManualReviewDate;
    ...
    return NextResponse.json(
      createErrorResponse('STORAGE_UNAVAILABLE', 'Failed to update source'),
      { status: 500 },
    );
  }
}
