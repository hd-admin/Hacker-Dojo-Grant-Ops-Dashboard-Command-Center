...
} from '@/server/grant-ops/crawl-scheduler-service';
...
export async function PUT(
  ...
  try {
    ...
    const schedule = await upsertScheduleForSource(
      sourceId,
    ...
    return NextResponse.json(
      createErrorResponse('STORAGE_UNAVAILABLE', 'Failed to save schedule'),
      { status: 500 },
    );
  }
}
...
export async function DELETE(
  ...
  try {
    ...
    return NextResponse.json(
      createErrorResponse('STORAGE_UNAVAILABLE', 'Failed to delete schedule'),
      { status: 500 },
    );
  }
}
