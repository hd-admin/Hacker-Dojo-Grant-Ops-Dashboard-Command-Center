...
describe('/api/outreach route', () => {
  ...
  describe('GET', () => {
    it('returns outreach records', async () => {
      ...
      (deps.repository.getOutreachRecords as ReturnType<typeof vi.fn>).mockResolvedValue(
        mockRecords,
      );
    ...
    it('filters outreach by grantId', async () => {
      ...
      const mockReq = {
        url: 'http://localhost:3000/api/outreach?grantId=g1',
  ...
  describe('POST', () => {
    it('returns 400 for invalid body', async () => {
      ...
      ) => Request)('http://localhost:3000/api/outreach', {
        method: 'POST',
        body: JSON.stringify({}),
      });
    ...
    it('returns 400 for missing grantId', async () => {
      ...
      const mockReq = new (NextRequest as unknown as new (
        ...
        method: 'POST',
        body: JSON.stringify({ contactName: 'Test' }),
      });
      const response = await POST(mockReq as unknown as NextRequest);
      expect(response.status).toBe(400);
    });
    ...
    it('creates outreach record with valid data', async () => {
      ...
      const mockReq = new (NextRequest as unknown as new (
        ...
        method: 'POST',
        body: JSON.stringify({ grantId: 'g1', contactName: 'Test', method: 'email' }),
      });
      const response = await POST(mockReq as unknown as NextRequest);
      const data = await (response as NextResponse).json();
