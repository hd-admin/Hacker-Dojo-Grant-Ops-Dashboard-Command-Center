...
describe('/api/eligibility-vetting route', () => {
  ...
  it('queues an eligibility vetting job', async () => {
    ...
    const req = new Request('http://localhost/api/eligibility-vetting', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({}) });
  ...
  it('queues with grantId', async () => {
    ...
    const req = new Request('http://localhost/api/eligibility-vetting', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ grantId: 'g1' }) });
    const response = await POST(req as unknown as NextRequest);
