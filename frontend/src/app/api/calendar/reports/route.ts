...
export async function GET(_request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    for (const award of awards as unknown as Array<{ id: string; title: string; funder: string }>) {
      const awardReports = ((await deps.repository.getReportDeadlinesByAwardId?.(award.id)) ??
    ...
    return NextResponse.json(createErrorResponse('STORAGE_UNAVAILABLE', 'Failed to get reports'), {
      status: 500,
    });
  }
}
