...
function buildGroundingSections(
  opencodeGroundingSections?: Array<{
    sectionTitle: string;
    evidence: string[];
    isGrounded: boolean;
  }>,
...
export async function generateDraft(
  ...
  const latestVersion =
    existingDrafts.length > 0 ? Math.max(...existingDrafts.map((d) => d.version)) : 0;
  ...
  if (!response.success && response.failureMode === 'partial-output' && response.content) {
    ...
    const nextSourceCount =
      groundedDocuments.length > 0 ? groundedDocuments.length : (grant.sourceCount ?? 0);
  ...
  if (!response.success || !response.content) {
    throw new Error(`Draft generation failed: ${response.error || 'Unknown error from Opencode'}`);
  }
...
export async function createRevisionRequest(
  ...
  const latestVersion = drafts.length > 0 ? Math.max(...drafts.map((d) => d.version)) : 0;
