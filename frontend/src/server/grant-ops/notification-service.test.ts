...
describe('notification-service', () => {
  ...
  describe('generateCrawlNotifications', () => {
    it('generates informational notification for successful crawl', () => {
      ...
      const notifications = service.generateCrawlNotifications(crawlRun, 15, 8);

      expect(notifications).toHaveLength(1);
      expect(notifications[0]!.text).toBe('Crawl completed: 15 grants found, 8 matched');
    ...
    it('generates warning notification for failed crawl', () => {
      ...
      const notifications = service.generateCrawlNotifications(crawlRun, 0, 0);
  ...
  describe('generateSubmissionNotifications', () => {
    it('generates notification for recorded submission', () => {
      ...
      const notifications = service.generateSubmissionNotifications(grant, submission);

      expect(notifications).toHaveLength(1);
      expect(notifications[0]!.text).toBe('Grant "Test Grant" submitted');
      expect(notifications[0]!.dot).toBe('info');
    });
  });
  ...
  describe('generateDeadlineNotifications', () => {
    it('generates notifications for grants with approaching deadlines', () => {
      ...
      const notifications = service.generateDeadlineNotifications(grants, profile);
  ...
  describe('generateFollowUpNotifications', () => {
    it('generates warning for pending follow-ups with due dates', () => {
      ...
      const notifications = service.generateFollowUpNotifications(followUps, grants);

      expect(notifications.length).toBeGreaterThanOrEqual(1);
      const followUpNotif = notifications.find((n) => n.text.includes('Follow-up due'));
    ...
    it('generates urgent notification for overdue follow-ups', () => {
      ...
      expect(notifications.length).toBeGreaterThanOrEqual(1);
      const overdueNotif = notifications.find((n) => n.text.includes('OVERDUE'));
    ...
    it('skips completed follow-ups', () => {
      ...
      const deadlineNotifs = notifications.filter((n) => n.text.includes('Follow-up'));
  ...
  describe('generateTaskNotifications', () => {
    it('generates notification for completed tasks', () => {
      ...
      expect(notifications).toHaveLength(1);
      expect(notifications[0]!.text).toBe('Task "Review budget section" completed');
    });
