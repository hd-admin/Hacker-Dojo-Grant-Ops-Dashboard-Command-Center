...
export async function executeQueuedJob(
  ...
  for (let attempt = 0; attempt <= MAX_AUTO_RETRIES; attempt++) {
    try {
      ...
      const isTransient =
        category === 'rate-limit' || category === 'timeout' || category === 'connectivity';

      // Preserve partial output from error object when present
      if (error !== null && typeof error === 'object' && 'partialOutput' in error) {
 ...
 * was not found.
 */
async function _restartQueuedJob(jobId: string): Promise<JobQueueItem | null> {
  const deps = getDependencies();
  const job = await deps.repository.getJobQueueItem(jobId);
 ...
 */
export async function updateJobProgress(jobId: string, stage: JobProgressStage): Promise<void> {
  const deps = getDependencies();
  await deps.repository.updateJobQueueItem(jobId, {
    stage,
    lastUpdate: now(),
