...
async function _getDashboardKpis(): Promise<DashboardKpis> {
  ...
  const activeGrants = allGrants.filter(
    (grant) => grant.status !== 'awarded' && grant.status !== 'archived',
  );
  ...
  const staleDays = lastRun
    ...
    : null;
  const isStale =
    lastRun === null || lastRun.status === 'failed' || (staleDays !== null && staleDays > 7);
...
export function exportPipelineToCsv(grants: Grant[]): CsvExport {
  const headers = ['Title', 'Funder', 'Award', 'Deadline', 'Status', 'Responsibility', 'Fit Score'];
...
export function generateAnnualSummary(grants: Grant[]): AnnualSummary {
  const totalGrantsSubmitted = grants.filter(
    (g) => g.status === 'submitted' || g.status === 'awarded',
  ).length;
  ...
  const lessonsLearned = grants
    .filter(
      (g) => g.status === 'declined' && g.lessonsLearned && g.lessonsLearned.trim().length > 0,
    )
    .map((g) => ({
      grantTitle: g.title,
      funder: g.funder,
