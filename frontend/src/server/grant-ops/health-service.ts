...
export async function checkStorage(
  deps: Dependencies,
): Promise<Pick<HealthCheckResult, 'storage' | 'storageError'>> {
  try {
...
async function performHandshake(
  ...
  Pick<
    HealthCheckResult,
    'handshakeSuccess' | 'handshakeResponseTimeMs' | 'handshakeError' | 'capabilities'
  >
> {
...
export async function checkCrawler(
  deps: Dependencies,
): Promise<Pick<HealthCheckResult, 'crawlerStatus' | 'crawlerLastRunAt'>> {
  try {
...
export async function checkDocumentIndexer(
  deps: Dependencies,
): Promise<
...
export function classifyRootCause(
  ...
  // Binary missing
  if (
    failureMode === 'install-missing' ||
    /command not found|no such file|enoent|not installed/i.test(lower)
  ) {
    return {
  ...
  // Network blocked
  if (
    failureMode === 'connectivity' ||
    /network.*unreachable|connection refused|econnrefused|enotfound|dns.*resolve/i.test(lower)
  ) {
    return {
  ...
  if (
    ...
    /503|service unavailable|overloaded|busy/i.test(lower)
  ) {
    return {
  ...
  // Quota depleted
  if (
    failureMode === 'quota-exhausted' ||
    /quota exceeded|quota exhausted|billing.*limit/i.test(lower)
  ) {
    return {
  ...
  // Session interrupted
  if (
    failureMode === 'interrupted-session' ||
    failureMode === 'partial-output' ||
    /session.*terminat|connection.*closed|stream.*interrupted/i.test(lower)
  ) {
    return {
