...
export const DEFAULT_NOTIFICATION_RULES: NotificationRule[] = [
  ...
  },
  {
    ...
    enabled: true,
    urgency: 'warning',
    template: 'Grant "{grantTitle}" deadline in {days} days ({deadlineConfidence})',
    desktopNotify: false,
  },
  {
...
export interface NotificationService {
   ...
   */
  generateDraftReadyNotification(grant: Grant, draft: DraftArtifact): Notification | null;

  /**
   * Generate notification from submission events
   */
  generateSubmissionNotifications(grant: Grant, submission: SubmissionRecord): Notification[];

  /**
   * Generate deadline proximity notifications for grants with approaching deadlines
   */
  generateDeadlineNotifications(grants: Grant[], profile: OrganizationProfile): Notification[];

  /**
   * Generate follow-up deadline notifications
   */
  generateFollowUpNotifications(followUps: FollowUp[], grants: Grant[]): Notification[];
   ...
   */
  applyRule(rule: NotificationRule, event: NotificationEvent): Notification | null;
...
export function createNotificationService(
  ...
  function getRule(eventType: NotificationEventType): NotificationRule | undefined {
  ...
  return {
    ...
    generateDraftReadyNotification(grant: Grant, _draft: DraftArtifact): Notification | null {
    ...
    generateSubmissionNotifications(grant: Grant, _submission: SubmissionRecord): Notification[] {
      const notifications: Notification[] = [];
    ...
    generateDeadlineNotifications(grants: Grant[], _profile: OrganizationProfile): Notification[] {
      const notifications: Notification[] = [];
    ...
    generateFollowUpNotifications(followUps: FollowUp[], _grants: Grant[]): Notification[] {
      const notifications: Notification[] = [];
      const now = clock.now();
      ...
      for (const followUp of followUps) {
        ...
        if (followUp.status === 'overdue' || followUp.dueDate) {
          const isOverdue =
            followUp.status === 'overdue' || (followUp.dueDate && new Date(followUp.dueDate) < now);
          ...
          if (rule) {
            const context: Record<string, string> = {
              ...
              date: followUp.dueDate ? new Date(followUp.dueDate).toLocaleDateString() : 'now',
            };
            notifications.push(
              createNotification(
    ...
    generateFromAuditEvents(events: AuditEvent[]): Notification[] {
      ...
      for (const event of events.slice(-10)) {
        // Only recent events
        const eventTypeMap: Partial<Record<string, NotificationEventType>> = {
        ...
        if (mappedType) {
          ...
          if (rule) {
            ...
            notifications.push(
              createNotification(interpolate(rule.template, metadata), rule.urgency),
            );
          }
        }
      }
    ...
    classifyUrgency(event: NotificationEvent): NotificationUrgency {
      ...
      if (event.type === 'deadline_proximity' && event.deadlineConfidence) {
    ...
    applyRule(rule: NotificationRule, event: NotificationEvent): Notification | null {
      ...
      const context: Record<string, string> = {
        ...
        date: event.deadline ? new Date(event.deadline).toLocaleDateString() : '',
        days: event.deadline
          ? String(
              Math.ceil(
                (new Date(event.deadline).getTime() - clock.now().getTime()) /
                  (1000 * 60 * 60 * 24),
              ),
            )
          : '',
      ...
      const urgency =
        event.type === 'deadline_proximity' && event.deadlineConfidence
          ? this.classifyUrgency(event)
          : rule.urgency;
      return createNotification(interpolate(rule.template, context), urgency, event.grantId);
    },
