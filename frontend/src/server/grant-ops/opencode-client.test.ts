...
`,
  ...
  describe('classifyOpencodeError', () => {
    ...
    it('classifies auth/permission errors as config-error', () => {
      expect(classifyOpencodeError('API key not set', 'Authentication failed')).toBe(
        'config-error',
      );
    ...
    it('classifies rate limiting as rate-limit', () => {
      ...
      expect(classifyOpencodeError('Rate limit exceeded', 'quota exceeded for today')).toBe(
        'rate-limit',
      );
    });

    it('classifies parse errors as malformed-output', () => {
      expect(classifyOpencodeError('SyntaxError: Unexpected token in JSON')).toBe(
        'malformed-output',
      );
    ...
    it('classifies token limit errors as context-overflow', () => {
      expect(classifyOpencodeError('Context length exceeded 128000 tokens')).toBe(
        'context-overflow',
      );
      expect(classifyOpencodeError('input is too long, maximum context window is 200k')).toBe(
        'context-overflow',
      );
    });
    ...
    it('classifies truncated output as partial-output', () => {
      ...
      expect(classifyOpencodeError('stream closed unexpectedly', 'broken pipe')).toBe(
        'partial-output',
      );
    });
    ...
    it('classifies quota exceeded as quota-exhausted', () => {
      expect(classifyOpencodeError('quota exceeded for this billing period')).toBe(
        'quota-exhausted',
      );
