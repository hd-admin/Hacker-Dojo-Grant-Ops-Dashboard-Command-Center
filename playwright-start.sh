...
env DATA_DIR="$DATA_DIR" PORT=3000 HOSTNAME=0.0.0.0 OPENCODE_BIN="${OPENCODE_BIN:-}" OPENCODE_PURE="${OPENCODE_PURE:-}" "$ROOT_DIR/node_modules/.bin/next" start &
START_PID=$!

READY=0
for _ in $(seq 1 90); do
  SERVER_PID="$(lsof -t -iTCP:3000 -sTCP:LISTEN 2>/dev/null | head -n 1 || true)"
