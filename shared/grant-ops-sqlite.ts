...
export function openDatabase(state: SqliteBootstrapState): SqliteDatabase {
  ...
  if (existsSync(walPath)) {
    try {
      ...
      const cpRow = tempDb.prepare('PRAGMA wal_checkpoint(PASSIVE)').get() as
...
export function checkIntegrity(state: SqliteBootstrapState): { ok: boolean; errors: string[] } {
  ...
  if (errors.length > 0) {
    const fullResult = db.pragma('integrity_check') as unknown as Array<{
      integrity_check: string;
    }>;
    const fullErrors = fullResult
      .filter((row) => row.integrity_check !== 'ok')
      .map((row) => row.integrity_check);
...
function loadTable<T>(db: SqliteDatabase, table: string): T[] {
  const rows = db.prepare(`SELECT json FROM ${table} ORDER BY rowid ASC`).all() as Array<{
    json: string;
  }>;
...
function loadSingleton<T>(db: SqliteDatabase, table: string, id: string): T | null {
  const row = db.prepare(`SELECT json FROM ${table} WHERE id = ? LIMIT 1`).get(id) as
    | { json: string }
    | undefined;
...
function seedDefaultState(db: SqliteDatabase): void {
  ...
  const existingBackupSchedule = db
    .prepare('SELECT 1 FROM backup_schedule WHERE id = 1 LIMIT 1')
    .get();
...
function bootstrapFromLegacy(
  ...
  const opencodeSettings =
    legacy.standaloneOpencode ?? legacy.persisted?.opencodeSettings ?? defaultOpencodeSettings;
...
export async function clearDatabase(state: SqliteBootstrapState): Promise<void> {
  ...
  await Promise.all(
    [state.dbPath, `${state.dbPath}-wal`, `${state.dbPath}-shm`, `${state.dbPath}-journal`].map(
      (filePath) => fs.rm(filePath, { force: true }),
    ),
  );
...
function getMigrationsDir(): string {
  return path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..', 'shared', 'migrations');
}
    ...
    for (const file of migrationFiles) {
      ...
      try {
        ...
        if (message.includes('duplicate column name')) {
          ...
          return {
            success: false,
            version: getCurrentSchemaVersion(state),
...
export function readAuditEvents(state: SqliteBootstrapState, limit?: number): AuditEvent[] {
  const db = openDatabase(state);
  const sql = limit
...
export function readJobQueue(state: SqliteBootstrapState, status?: string): JobQueueItem[] {
  const db = openDatabase(state);
  const rows = db.prepare('SELECT * FROM job_queue ORDER BY created_at DESC').all() as Array<
  ...
  const items = rows.map((row) => {
    ...
    if (row.failure_category)
      item.failureCategory = String(row.failure_category) as JobFailureCategory;
    return item;
  });
...
export function updateJobQueueItem(
  state: SqliteBootstrapState,
  id: string,
...
export function readDuplicateCandidates(
  ...
  const db = openDatabase(state);
  const rows = db
    .prepare('SELECT * FROM duplicate_candidates ORDER BY detected_at DESC')
...
export function saveDuplicateCandidate(
  state: SqliteBootstrapState,
...
export function readConflictRecords(
  state: SqliteBootstrapState,
  grantId?: string,
): ConflictRecord[] {
  const db = openDatabase(state);
  const rows = db.prepare('SELECT * FROM conflict_records ORDER BY field_name ASC').all() as Array<
    Record<string, unknown>
  >;
  const items = rows.map((row) => {
...
export function readSubmissionManifests(
  state: SqliteBootstrapState,
  grantId?: string,
): SubmissionManifest[] {
  const db = openDatabase(state);
  const rows = db
    .prepare('SELECT * FROM submission_manifests ORDER BY created_at DESC')
    .all() as Array<Record<string, unknown>>;
  const items = rows.map((row) => {
...
export function saveSubmissionManifest(
  state: SqliteBootstrapState,
...
export function removeApprovalRecordByGrantId(state: SqliteBootstrapState, grantId: string): void {
  const db = openDatabase(state);
  const records = loadTable<ApprovalRecord>(db, 'approval_records').filter(
    (record) => record.grantId !== grantId,
  );
...
export function saveBackupVerificationRecord(
  state: SqliteBootstrapState,
...
export function saveBackupFreshness(
  state: SqliteBootstrapState,
...
export function readThemesData(state: SqliteBootstrapState): ThemesData {
  const db = openDatabase(state);
  const row = db.prepare("SELECT json FROM themes_data WHERE id = 'themes_data' LIMIT 1").get() as
    | { json: string }
    | undefined;
...
export function writeAwardBudgetCategories(
  state: SqliteBootstrapState,
  categories: AwardBudgetCategory[],
): void {
  const db = openDatabase(state);
...
export function writePlannedExpenses(
  state: SqliteBootstrapState,
...
export function writeAwardReportDeadlines(
  state: SqliteBootstrapState,
  deadlines: AwardReportDeadline[],
): void {
  const db = openDatabase(state);
...
export function writeAwardComplianceItems(
  state: SqliteBootstrapState,
  items: AwardComplianceItem[],
): void {
  const db = openDatabase(state);
...
export function writePeerDiscoveryResults(
  state: SqliteBootstrapState,
  results: PeerDiscoveryResult[],
): void {
  const db = openDatabase(state);
...
export function readPipelineTransitions(
  state: SqliteBootstrapState,
  grantId?: string,
): PipelineTransition[] {
  const db = openDatabase(state);
...
export function writePipelineTransition(
  state: SqliteBootstrapState,
...
export function readFormTemplates(state: SqliteBootstrapState): Array<{
  id: string;
  name: string;
  ...
  const db = openDatabase(state);
  const rows = db
    .prepare('SELECT * FROM application_form_templates ORDER BY createdAt DESC')
    .all() as Array<Record<string, unknown>>;
  return rows.map((row) => ({
    id: String(row.id),
...
export function readOutreachRecords(
  state: SqliteBootstrapState,
  grantId?: string,
): Array<{
  id: string;
...
export function writeOutreachRecord(
  state: SqliteBootstrapState,
  record: {
    id: string;
    grantId: string;
...
export async function getDbDiagnostics(
...
): Promise<{ status: string; walEnabled: boolean }> {
  const dbPath = path.join(dataDir, 'grant-ops.sqlite');
  if (!existsSync(dbPath)) {
    return { status: 'no-db', walEnabled: false };
  }
  ...
  try {
    ...
    const checkRow = db.prepare('PRAGMA integrity_check').get() as
      | { integrity_check: string }
      | undefined;
    const status = checkRow?.integrity_check === 'ok' ? 'ok' : 'error';

    const walRow = db.prepare('PRAGMA journal_mode').get() as { journal_mode: string } | undefined;
    const walEnabled = walRow?.journal_mode === 'wal';

    return { status, walEnabled };
  } catch {
    return { status: 'error', walEnabled: false };
  } finally {
    if (db) db.close();
  }
}
